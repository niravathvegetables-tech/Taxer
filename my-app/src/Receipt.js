...
import url from './Config';
 ...
 componentDidMount() {
    this.fetchReceipts(); // called once when component mounts
  }
  ...
  try {
    ...
    let receipts = [];
    if (data.receipt) {
      // Normalize: if it's already an array, use it; if it's a single object, wrap it
          ...
          {receipt.length === 0 ? (
            ...
            receipt.map((r) => (
              <tr key={r.receipt_id}>
                ...
                <td>
                  <button className="btn-delete" onClick={() => this.handleDelete(r.receipt_id)}>Delete</button>
                </td>
              </tr>
            ))
          )}
